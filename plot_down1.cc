...
*/        
        ...
        if(ht.ok) {
          ...
          if(idx==2 && run_number<=364485) h[0][0]->Fill(ct_reco, wt);
          if(idx==2 && run_number>=431810) h[1][0]->Fill(ct_reco, wt);
          if(idx==2) h[2][0]->Fill(ct_reco, wt);
        }
      }
    }
  }
  ...
  for(Long64_t entry=0; entry<nentries3; entry++) {
    ...
    //if(truth_isBstarDecay || !passFiducial(false) || !passFilter(false)) continue;
    if(truth_isBstarDecay || !passFiducial(false)) continue;
    ...
    for(Int_t i=0; i<truth_n; i++) {
      if(truth_pdgId[i]==35) {
        //if(truth_pT[i]/GeV>15 && fabs(truth_eta[i])<2.5) {//fiducial
  ...
  for(Int_t i=0; i<3; i++) {
    ...
    if(h[i][1]->Integral()>0 && h[i][0]->Integral()>0) h[i][1]->Scale(h[i][0]->Integral() * 2 / h[i][1]->Integral());
    h[i][0]->Scale(18.5);
    h[i][1]->Scale(18.5); 
    ...
    h[i][0]->SetYTitle(str);
    h[i][0]->Draw("hist");
    h[i][1]->SetLineColor(kBlack); h[i][1]->SetLineWidth(3);
    ...
    leg[i]->AddEntry(h[i][1], "AOD.Truth.", "l");     
    //leg[i]->AddEntry(h[i][1], "Fiducial", "l");     
    leg[i]->AddEntry(h[i][0], "Baseline", "l");
    leg[i]->Draw();
