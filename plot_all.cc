...
void plot() {
  ...
  for(Long64_t entry=0; entry<nentries3; entry++) {
    ...
    if(passFilter(false)) {
      ...
      if(passFiducial(false)) {
        if(ch[0]->GetTreeNumber()<=3) h[0][2]->Fill(ct_truth,weight); 
        if(ch[0]->GetTreeNumber()>3) h[1][2]->Fill(ct_truth,weight);
        h[2][2]->Fill(ct_truth,weight);   
      }
    }
  }
  ...
  for(Int_t i=0; i<3; i++) {
    ...
    if(i==0) leg[i] = new TLegend(0.6, 0.8, 0.9, 0.9);
    else if(i==1) leg[i] = new TLegend(0.6, 0.7, 0.9, 0.8);
    else leg[i] = new TLegend(0.6, 0.6, 0.9, 0.7);
  }
  ...
  if(read_ratio) { 
    ...
    for(Int_t i=0; i<3; i++) {
      for(Int_t j=1; j<=h_ratio[i]->GetNbinsX(); j++) {
        ...
        Float_t tmp1_2 = h[i][2]->GetBinContent(j)>0 ? h[i][2]->GetBinError(j)/h[i][2]->GetBinContent(j) : 0;
        Float_t tmp2;
        ...
        if (eps <= 0) continue;
        const Float_t tmp_1 = h[i][2]->GetBinContent(j) / h[i][0]->GetBinContent(j) * eps;
        h_ratio[i]->SetBinContent(j,tmp_1);
        h_ratio[i]->SetBinError(j,tmp_1*sqrt(pow(tmp1_1,2)+pow(tmp1_2,2)+pow(tmp2,2)));
      }
    }
  ...
  c[0] = new TCanvas(str,str,600,600);
  ...
  for(Int_t i=0; i<3; i++) {
    ...
    //double ymax = TMath::Max(h[i][0]->GetMaximum(), h[i][1]->GetMaximum());
      ...
      else if(i==1) h_ratio[i]->SetLineColor(kRed);
      else h_ratio[i]->SetLineColor(kBlack); 
      ...
      else if(i==1) h_ratio[i]->Draw("EP SAME");
      ...
      else if(i==1) leg[i]->AddEntry(h_ratio[i], "Run3 Eff.", "lep");
      else leg[i]->AddEntry(h_ratio[i], "Run2+Run3 Eff.", "lep");
      leg[i]->Draw();
