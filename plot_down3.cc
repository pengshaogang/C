...
void plot() {
  ...
  //ch[2]->Add("./data24_BphysDelayed_baseline_noC.root");
  Init(ch[2]);  
  ...
  for(Int_t ich=2; ich<3; ich++) {
	...
	dR_B0Ks = tmpJX.DeltaR(tmpKs);
	    ...
	    if(run_number<=364485) h[0][0]->Fill(ct_reco);
            ...
            //if(run_number>=431810 && run_number<=456749) h[1][0]->Fill(ct_reco);
            if(run_number>=473235 && run_number<=486706) h[1][0]->Fill(ct_reco);
	    h[2][0]->Fill(ct_reco);
	  }
	  if(idx==4) {
	    if(run_number<=364485) h[3][0]->Fill(ct_reco,wt);
	    //if(run_number>=431810) h[4][0]->Fill(ct_reco,wt);
	    //if(run_number>=431810 && run_number<=456749) h[4][0]->Fill(ct_reco,wt);
	    if(run_number>=473235 && run_number<=486706) h[4][0]->Fill(ct_reco,wt);
